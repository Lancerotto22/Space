...
function DetailsView({ stages, trackerEvents }: DetailsViewProps) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR("customers/${customer}/projects/${project}/deliverables/${deliverable}", async () => {
    ...
    <>
      <Grid numItemsMd={1} numItemsLg={3} className="gap-6 mt-6">
        {Object.keys(Stages).length === 0 ? (
          ...
          Object.entries(Stages).map(([stageName, stageInfo]) => (
            <StageCard key={stageName} stageName={stageName} info={stageInfo} uploads={uploads} />
          ))
        )}
      </Grid>
